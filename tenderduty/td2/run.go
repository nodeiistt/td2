...
func Run(configFile, stateFile, chainConfigDirectory string, password *string) error {
	...
	for k := range td.Chains {
		...
		go func(cc *ChainConfig, name string) {
			...
			go func() {
				for {
					// Check if this is a Gnoland chain and use specialized health checks
					if cc.IsGnolandChain() {
						cc.gnolandHealthCheck(td.ctx)
					} else {
			...
			for {
				...
				// Check if this is a Gnoland chain and use specialized monitoring
				if cc.IsGnolandChain() {
					cc.gnolandMonitorBlocks(td.ctx)
				} else {
				cc.WsRun()
				}
				l(cc.ChainId, "🌀 monitoring exited! Restarting monitoring")
				time.Sleep(5 * time.Second)
			}
		}(cc, k)
	}
