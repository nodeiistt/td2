...
func (cc *ChainConfig) newRpc() error {
	// Check if this is a Gnoland chain and use specialized functions
	if cc.IsGnolandChain() {
		return cc.gnolandNewRpc()
	}
	...
	tryUrl := func(u string) (msg string, down, syncing bool) {
		...
		// For Union chain, use getStatusWithEndpoint to handle BN254 pubkey
		var network string
		var catching_up bool
		
		// Try the new getStatusWithEndpoint function first (for Union chain support)
		...
		if err != nil {
			...
			// Still need to create the client for websocket connection
			cc.client, err = rpchttp.New(u, "/websocket")
			if err != nil {
				msg = fmt.Sprintf("❌ could not connect client for %s: (%s) %s", cc.name, u, err)
				l(msg)
...
// getStatusWithEndpoint handles TCP schema and other endpoint types for Union chain
